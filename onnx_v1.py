...
def process_rtsp_stream(rtsp_url, model):
    ...
    while True:
        ...
        # 推理处理
        detections = inference_on_image(model, frame)
        # 绘制检测结果
        frame_with_detections = draw_detections(frame, detections)
        # 显示结果
        cv2.imshow('RTSP Stream', frame_with_detections)
...
if __name__ == '__main__':
    # 模型文件的路径
    model_path = r"D:\YOLOv8_data\YOLO_tools\yolov8-streamlit-detection-tracking\weights\0802_ppl_car.onnx"
    ...
    if mode == 1:
        # 读取图像文件
        image_data = cv2.imread(r"D:\YOLOv8_data\0802_ppl_car_split\images\val\00000005.jpg")
        # 使用检测模型对读入的图像进行对象检测
        ...
        # New mode for processing all images in a folder
        input_folder = r"D:\YOLOv8_data\0802_ppl_car_split\images\val"  # Change to your input folder path
        output_folder = r"D:\YOLOv8_data\output_images"  # Change to your output folder path
        ...
        for image_name in os.listdir(input_folder):
            ...
            if image_path.endswith(('.jpg', '.jpeg', '.png', '.bmp')):  # Adjust extensions as needed
                ...
                print(f"Inference time for {image_name}: {inference_time:.4f} seconds")
    else:
